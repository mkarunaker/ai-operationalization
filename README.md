...
The browser never receives a key. The committed provider policy is [src/config/model-routing.ts](src/config/model-routing.ts), while model IDs remain in `.env.local`; capability roles therefore remain separate from providers and model names. Strategist, Skeptic, Editor, Synthesizer, both drafters, and Proofreader use low reasoning. Reviewer and Synthesizer allowances default to 1,600 tokens and remain bounded server-only settings. Initial and Final Drafter allowances scale from the saved target range: ordinary article and short-post ranges keep 2,400 and 1,600 tokens respectively, while deeper requested outputs reserve more room up to 9,000 tokens. The Proofreader defaults to 1,600 tokens. Optional fixed overrides are `EDITORIAL_REVIEWER_MAX_OUTPUT_TOKENS` (1,200–3,000), `EDITORIAL_SYNTHESIZER_MAX_OUTPUT_TOKENS` (1,600–3,000), `EDITORIAL_INITIAL_DRAFTER_MAX_OUTPUT_TOKENS` (2,000–9,000), `EDITORIAL_FINAL_DRAFTER_MAX_OUTPUT_TOKENS` (1,200–9,000), and `EDITORIAL_PROOFREADER_MAX_OUTPUT_TOKENS` (1,200–3,000). Leave the drafter overrides unset to retain range-aware scaling. Restart before starting a new Board run after changing an allowance. Each Board snapshot records the exact drafting allowances it used.

Current official OpenAI prices are built in as operator-maintained estimates and may be overridden with matching `OPENAI_{LOW|MEDIUM|HIGH}_{INPUT|CACHED_INPUT|OUTPUT}_USD_PER_MILLION` values. Before each live Board run, the review screen displays every planned provider/model, pricing basis, estimate, quality profile, and editable budget cap. It refuses to dispatch when the upper-bound reservation exceeds the selected cap, which can never exceed $0.75. A requested 5,000-word Sol/Frontier run currently exceeds that ceiling once the permitted same-route structured repair is reserved; the app blocks it before dispatch and asks the author to reduce the range or choose a lower-cost route. A higher-tier rerun remains a one-role action and retains the original Board and draft.
