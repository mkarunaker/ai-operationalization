...
- Supported publication plans are LinkedIn only, Medium only, Substack only, or a canonical Medium/Substack piece with an optional LinkedIn companion.
- A 3–4 minute Medium/Substack piece is the canonical draft. A 1–2 minute LinkedIn version is a deliberately requested companion that links or leads into it; it is never silently generated.
- LinkedIn-only remains a first-class, simpler path.
- Platform defaults: LinkedIn; audience: professionals across AI, data, technology, business, and leadership; tone: neutral, thoughtful, practical, skeptical without cynicism; posture: observation and invitation; style: `kk-spoken-voice`.
...
→ add notes, source links, or a draft; choose publication plan
...
| Publication | canonical/companion relationship, platform, title, final text, URL, published date |
...
| Add | Themes, priority/reordering, publication-plan/canonical-companion relation | New thin migrations and queue UI |
...
| Defer | Full analytics, automated publishing, LinkedIn scraping, automated trends, fine-tuning | Not needed to publish next week |
...
3. **Avoid a two-draft tax.** A LinkedIn companion should be created only after the canonical long-form direction is accepted, and only when requested. Otherwise the workflow becomes twice the work.
...
| Canonical long-form platform | User chooses Medium or Substack per idea | Avoids premature account-level assumptions |
