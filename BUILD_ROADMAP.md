...
- Add dependency-safe recovery choices for a failed stage: retry unchanged; raise only the cap and retry after a new reservation; or create an explicit one-attempt output-allowance override that regenerates only the failed draft stage from its immutable saved Board snapshot. An override is recorded as distinct provenance and never modifies Settings, historical attempts, completed reviewers, or synthesis. Changes to the reader contract, source inputs, model route, or editorial reasoning require a new Board run. Do not offer arbitrary stage checkboxes.
