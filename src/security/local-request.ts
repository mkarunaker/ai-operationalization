...
export function safeRouteError(error: unknown) {
  ...
  const safePatterns = [
    ...
    /^A positive per-run budget cap is required for the working-draft retry\./,
    /^A valid working-draft retry budget cap is required\./,
    ...
    /^The working-draft retry cap cannot exceed/,
    ...
    /^Working-draft recovery must use the configured( low-cost)? Initial Drafter route\./,
    ...
    /^Only one working-draft retry is permitted for a saved Editorial Board run\./,
    /^The saved Editorial Board (recovery snapshot is invalid|synthesis is unavailable)\. Run the Editorial Board again before retrying the draft stage\./,
    ...
    /^The configured Initial Drafter route has changed since this Board run\. Run the Editorial Board again before retrying the working draft\./,
    ...
    /^New supporting visual authoring is deferred while /,
