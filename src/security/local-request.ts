...
export function safeRouteError(error: unknown) {
  ...
  const safePatterns = [
    ...
    /^The configured model declined the structured LinkedIn drafting request\./,
    ...
    /^The LinkedIn drafter (reached its output limit|returned an invalid structured response|completed, but its result could not be saved safely|failed before a validated response was available)/,
    /^Only (the explicit|an explicit) LinkedIn escalation /,
    /^The explicit LinkedIn escalation action uses /,
    /^Explain why this LinkedIn recovery needs /,
