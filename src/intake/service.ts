...
function focusedQuestions(notes: string): string[] {
  ...
  const candidates = [
    ...
    ["What reader context, tone, or length should shape it?", /(reader|audience|distribution|tone|length)/],
  ] as const;
...
function buildBrief(notes: string, answers: Array<{ question: string; answer: string; choice: string }>, bestJudgment: boolean): z.infer<typeof briefInput> {
  ...
  return {
    ...
    possibleCounterargument: null, desiredTone: find("tone") ?? (bestJudgment ? "Direct, clear, and thoughtful" : null), readerContext: find("reader context") ?? null,
...
export function completeIntake(ideaId: string, input: unknown) {
  ...
  try {
    ...
    try {
      ...
      db.prepare(`INSERT INTO content_intent_briefs (id, content_item_id, working_title, central_thesis, intended_audience, purpose, trigger_context, supporting_points, user_provided_evidence, claims_requiring_validation, possible_counterargument, desired_tone, reader_context, suggested_length, relationship_to_previous_posts, open_questions, system_assumptions, version, status) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'editable')`).run(briefId, row.content_id, brief.workingTitle ?? null, brief.centralThesis ?? null, brief.intendedAudience ?? null, brief.purpose ?? null, brief.triggerContext ?? null, JSON.stringify(brief.supportingPoints), JSON.stringify(brief.userProvidedEvidence), JSON.stringify(brief.claimsRequiringValidation), brief.possibleCounterargument ?? null, brief.desiredTone ?? null, brief.readerContext ?? null, brief.suggestedLength ?? null, brief.relationshipToPreviousPosts ?? null, JSON.stringify(brief.openQuestions), JSON.stringify(brief.systemAssumptions), version);
